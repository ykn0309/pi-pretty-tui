...
- `clean`: show a supported tool with the normal `full` renderer while it is running; after completion, merge it into a live status such as `Working(5 tool calls · thinking...)`. The activity slot changes to the current supported tool while one is running, and to `done` after the run settles. Visible assistant text starts a new tool group, so later calls get a separate status. Completed thinking blocks stay hidden, while active thinking shows `Thinking...`; press `Ctrl+O` to reveal thinking content and normal expanded tool calls/output in transcript order.
