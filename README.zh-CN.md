...
- **可折叠的工具活动：** 不再让大量独立工具调用填满对话。工作进行时，受支持的工具会集中显示为一行 `Running(...)`；工作结束后，则变为低调的 `Done(...)`。点击分组可展开紧凑的子工具列表，点击任一子工具可查看完整详情，再次点击父级即可全部折叠。
- **清晰的对话结构：** assistant 文本、steer 消息和 compaction 摘要会形成自然的分组边界。即使任务很长或中途被打断，工具活动仍会停留在 transcript 中正确的位置；思考详情默认保持隐藏，需要时再展开。
- **按需查看有效信息：** 针对不同工具设计的摘要，会简洁呈现路径、命令、结果、diff、文件预览和实时 shell 输出，同时保留查看原始完整内容的能力。
- **统一、精致的视觉体验：** 圆角用户消息与输入框、主题感知配色、更清爽的 Markdown 列表，以及带语法高亮的圆角代码块，让整个 TUI 更协调。在 fullscreen 模式下，每个围栏代码块还带有符合原生体验的 `[Copy]` 控件。

可折叠分组覆盖 Pi 内置的 `read`、`bash`、`edit`、`write`、`grep`、`find` 和 `ls` 工具。第三方工具保留自己的渲染方式，同时不会影响 Pi 原有的光标、输入法、自动补全、鼠标交互和工具执行行为。
...
点击 `Running(...)` 或 `Done(...)` 行，可以保留父级并展开其下方的工具。点击单个子工具可切换完整视图，再次点击父级可折叠整个分组。任何时候都可以按 `Ctrl+O`，使用 Pi 的全局展开控制来显示或隐藏工具输出和思考内容。
